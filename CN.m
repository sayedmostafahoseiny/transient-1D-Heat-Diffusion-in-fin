...
for i=2:n ; % interior nodes
    A(i,i)= -2 *(1+B) - deltat*(m*L)^2 ;
    A(i,i-1)= B;
    A(i,i+1)= B ;
    b(i,1)= -B*(theta(j,i+1)+theta(j,i-1))+(2*(-1+B)+deltat*(m*L)^2)*theta(j,i) ;
end
...
xlabel( ' x (m) ' )
ylabel( ' theta ' )
legend ([plot1 plot2],{'numerical (Crank-Nicolson)','Analitical solution'}) % validation
